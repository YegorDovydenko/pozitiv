...
namespace EdgeCore\PostTypes\Slider\Shortcodes;

use EdgeCore\Lib;

/**
 * Class Slider
 * @package EdgeCore\PostTypes\Slider\Shortcodes
 */
class Slider implements Lib\ShortcodeInterface {
    ...
    public function render($atts, $content = null) {
        ...
        if ($slider != "") {
            ...
            if($ratio_edge_options['smooth_page_transitions'] == "yes" && $ratio_edge_options['smooth_pt_spinner_type'] != "") {
                $ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">' . ratio_edge_loading_spinners(true) . '</div></div>';
            }else{
                $ajax_loader = '<div class="edgtf-st-loader"><div class="edgtf-st-loader1">'. ratio_edge_loading_spinner_pulse() .'</div></div>';
            }

            /* set slider preloader - end */

            /* set padding for slider arrows - start */
            $slider_arrows_padding = "padding-top: " . esc_attr($this->ratio_edge_get_slider_navigation_padding()) . "px;";
            /* set padding for slider arrows - end */

            $html .= '<div id="edgtf-' . esc_attr($slider) . '" ' . $slider_anchor . ' class="carousel slide ' . esc_attr($animation_type_class . ' ' . $full_screen_class . ' ' . $responsive_height_class . ' ' . $height_class . ' ' . $auto_start_class . ' ' . $header_effect_class . ' ' . $slider_numbers_class .  ' ' . $slider_thumbs_class) . ' " ' . $slide_animation_timeout . ' ' . $data_height . ' ' . $data_parallax_effect . ' '.ratio_edge_get_inline_style($slide_holder_height).'><div class="edgtf-slider-preloader">'.$ajax_loader.'</div>';
            $html .= '<div class="carousel-inner ' . esc_attr($slider_css_position_class) . '" '.ratio_edge_get_inline_style($carouselinner_height).' '.$data_parallax_transform.'>';
            ...
            if (have_posts()) : $postCount = 0;
                while (have_posts()) : the_post();
                    ...
                    if ($slide_type == 'video') {
                        ...
                        $html .= '<div class="edgtf-video"><div class="edgtf-mobile-video-image" '.ratio_edge_get_inline_style('background-image: url(' . esc_url($video_image) . ')').'></div><div class="edgtf-video-overlay';
                        ...
                        $html .= '<div class="edgtf-image" '.ratio_edge_get_inline_style('background-image:url(' . esc_url($image) . ')').'>';
                        if ($slider_thumbs == 'no') {
                            $html .= '<img src="' . esc_url($image) . '" alt="' . esc_attr($title) . '">';
                        }

                        if ($image_overlay_pattern !== "") {
                            $html .= '<div class="edgtf-image-pattern" '.ratio_edge_get_inline_style('background: url(' . esc_url($image_overlay_pattern) . ') repeat 0 0').'></div>';
                        }
                        $html .= '</div>';
                    }
                    ...
                    if ($element_positions == 'custom') {
                        $html .= 
                            '<div class="edgtf-slider-elements-container edgtf-custom-elements '.( $frame_in_grid == 'yes' ? 'edgtf-grid' : '' ).'" '.ratio_edge_get_inline_style( ($frame_in_grid == 'no' && (($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_width", true)) != '') ? 'width: '.$meta_temp.'%; ' : '') . 'padding-top: ' . esc_attr($this->ratio_edge_get_slider_navigation_padding()) . 'px;').' '.$slide_elements_data_start.' '.$slide_elements_data_end.($frame_in_grid == 'yes' ? '' : ' data-width-mobile="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_mobile', true)).'" data-width-tablet-p="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_p', true)).'" data-width-tablet-l="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_l', true)).'" data-width-laptop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_laptop', true)).'" data-width-desktop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_desktop', true)).'"').'>'.
                                '<div class="edgtf-slider-elements-container-inner" ' . ratio_edge_get_inline_style((($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_height", true)) != '') ? 'padding-bottom: '.$meta_temp.'%;' : '') . '">' .
                                    '<div class="edgtf-slider-elements-holder-frame" ' . ratio_edge_get_inline_style('width: 100%; height: 100%').' data-default-width="'.esc_attr($def_width).'">'
                        ;
                    }
                    else {
                        $html .= 
                            '<div class="edgtf-slider-elements-container" '.ratio_edge_get_inline_style('width: 100%; box-sizing: border-box; height: 100%; ' . 'padding-top: ' . esc_attr($this->ratio_edge_get_slider_navigation_padding()) . 'px;').' '.$slide_elements_data_start.' '.$slide_elements_data_end.($frame_in_grid == 'yes' ? '' : ' data-width-mobile="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_mobile', true)).'" data-width-tablet-p="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_p', true)).'" data-width-tablet-l="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_tablet_l', true)).'" data-width-laptop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_laptop', true)).'" data-width-desktop="'.esc_attr(get_post_meta(get_the_ID(), 'edgtf_slide_holder_frame_width_desktop', true)).'"').'>'.
                                '<div class="edgtf-slider-elements-container-inner" '.ratio_edge_get_inline_style('height: 100%;').'>' .
                                    '<div class="edgtf-slider-elements-holder-frame '.( $frame_in_grid == 'yes' ? 'edgtf-grid' : '' ).'" '.ratio_edge_get_inline_style( ($frame_in_grid == 'no' && (($meta_temp = get_post_meta(get_the_ID(), "edgtf_slide_holder_frame_width", true)) != '') ? 'width: '.$meta_temp.'%; ' : '').'top: 50%; left: 50%; -webkit-transform: translate(-50%,-50%); transform: translate(-50%,-50%);').' data-default-width="'.esc_attr($def_width).'">'
                        ;
                    }
                                ...
                                case 'text':
                                    ...
                                    if ($text_options == 'advanced') {
                                        ...
                                        if ($slide_element['slideelementtextbgndcolor'] != '') {
                                            ...
                                            $style_string .= 'background-color: '.ratio_edge_rgba_color($slide_element_bg_color, $slide_element_bg_transparency).';';
                                        }
                                ...
                                case 'button':
                                    ...
                                    if ($slide_element['slideelementbuttoniconpack'] != '') {
                                        ...
                                        if ($button_params['icon_pack'] != 'no_icon') {
                                            $iconPackName = ratio_edge_icon_collections()->getIconCollectionParamNameByKey($button_params['icon_pack']);
                                    ...
                                    $slide_element_html .= '<div class="'.$class_string.'" '.$data_string.' style="'.$style_string.'"><div class="edgtf-slide-element-inner" style="'.$inner_style_string.'">'.ratio_edge_get_button_html($button_params).'</div></div>';
                                break;
                ...
                elseif(isset($slider_meta['slider_nav_thumbs']) && $slider_meta['slider_nav_thumbs'] == 'yes'){
                    ...
                    if (have_posts()) : $postCount = 0;
                        while (have_posts()) : the_post();
                            ...
                            $slide_type = get_post_meta(get_the_ID(), "edgtf_slide_background_type", true);
                            if( $slide_type == 'image'){
                                $img .= ratio_edge_generate_thumbnail('',esc_url(get_post_meta(get_the_ID(), "edgtf_slide_image", true)),'160','100');
                            }elseif($slide_type == 'video'){                                
                                $img .= ratio_edge_generate_thumbnail('',esc_url(get_post_meta(get_the_ID(), "edgtf_slide_video_image", true)),'160','100');
                            }                           
                            $html .= '>';
                            $html .= $img;
                            $html .= '</li>';
     ...
     **/
    function ratio_edge_get_slider_item_padding() {
        return apply_filters('ratio_edge_slider_item_padding', 0);
    }

    /**
     * Function that returns slider navigation pading
     **/
    function ratio_edge_get_slider_navigation_padding() {
        return apply_filters('ratio_edge_slider_navigation_padding', 0);
    }
}