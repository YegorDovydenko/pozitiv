...
class RatioEdgeImport {
    ...
    public function import_options($file){
        $options = $this->file_options($file);
        update_option( 'edgt_options_ratio', $options);
    ...
    function edgt_admin_import() {
        if (edgt_core_theme_installed()) {
            ...
            $this->pagehook = add_submenu_page(
                ...
                array($ratio_edge_Framework->getSkin(), 'renderImport')
            );

            add_action('admin_print_scripts-'.$this->pagehook, 'ratio_edge_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$this->pagehook, 'ratio_edge_enqueue_admin_styles');
...
function edgt_init_import_object(){
    global $ratio_edge_import_object;
    $ratio_edge_import_object = new RatioEdgeImport();
}

add_action('init', 'edgt_init_import_object');

if(!function_exists('ratio_edge_dataImport')){
    function ratio_edge_dataImport(){
        global $ratio_edge_import_object;

        if ($_POST['import_attachments'] == 1)
            $ratio_edge_import_object->attachments = true;
        else
            $ratio_edge_import_object->attachments = false;

        $folder = "ratio/";
        if (!empty($_POST['example']))
            $folder = $_POST['example']."/";

        $ratio_edge_import_object->import_content($folder.$_POST['xml']);

        die();
    }

    add_action('wp_ajax_edgt_dataImport', 'ratio_edge_dataImport');
}

if(!function_exists('ratio_edge_widgetsImport')){
    function ratio_edge_widgetsImport(){
        global $ratio_edge_import_object;
        ...
        $ratio_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');

        die();
    }

    add_action('wp_ajax_edgt_widgetsImport', 'ratio_edge_widgetsImport');
}

if(!function_exists('ratio_edge_optionsImport')){
    function ratio_edge_optionsImport(){
        global $ratio_edge_import_object;
        ...
        $ratio_edge_import_object->import_options($folder.'options.txt');

        die();
    }

    add_action('wp_ajax_edgt_optionsImport', 'ratio_edge_optionsImport');
}

if(!function_exists('ratio_edge_otherImport')){
    function ratio_edge_otherImport(){
        global $ratio_edge_import_object;
        ...
        $ratio_edge_import_object->import_options($folder.'options.txt');
        $ratio_edge_import_object->import_widgets($folder.'widgets.txt',$folder.'custom_sidebars.txt');
        $ratio_edge_import_object->import_menus($folder.'menus.txt');
        $ratio_edge_import_object->import_settings_pages($folder.'settingpages.txt');

        die();
    }

    add_action('wp_ajax_edgt_otherImport', 'ratio_edge_otherImport');
}