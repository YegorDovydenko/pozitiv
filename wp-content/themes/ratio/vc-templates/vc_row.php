...
if( ! empty($background_gradient) && $background_gradient == 'yes'){
	$css_classes[] = 'edgtf-row-gradient';

	if( ! empty($gradient_first_color) && ! empty($gradient_second_color)){
		$wrapper_style = ratio_edge_inline_background_gradient($gradient_first_color, $gradient_second_color);
	}
...
if( ! empty($row_type) && $row_type == 'parallax'){
	...
	if(ratio_edge_options()->getOptionValue('parallax_on_off') == 'off'){
...
if($row_type == 'row' && $css_animation != ''){
	...
	if($transition_delay !== ''){
		...
		$inner_start .= '<div '.ratio_edge_get_inline_style($animation_styles).'>';
		$inner_end .= '</div>';
	}else{
		$inner_start .= '<div>';
		$inner_end .= '</div>';
...
if($full_screen_section_height == 'yes'){
	...
	$scrol_icon = '';
	if($icon_scroll_down == 'yes') {
		$scrol_icon = '<div class="edgtf-scroll-down-icon"><span class="arrow_down"></span></div>';
	}

	$before_wrapper_close .= $scrol_icon . '</div>';
...
if($video == 'show_video'){
	...
	if($video_image) {
		$video_output .= '<div class="edgtf-mobile-video-image" ' . ratio_edge_get_inline_attr($video_mobile_style, 'style') . ')"></div>';
	}
	$video_output .= '<div ' . ratio_edge_get_class_attribute($video_overlay_class) . ratio_edge_get_inline_attr($video_overlay_style, 'style') . '></div>';
	$video_output .= '<div class="edgtf-video-wrap">';
	$video_output .= '<video class="edgtf-video" width="1920" height="800" '. ratio_edge_get_inline_attr($video_attrs, 'poster')  .' controls="controls" preload="auto" loop autoplay muted>';
	...
	if($v_image) {
		$video_output .= '<img ' . ratio_edge_get_inline_attr($v_image, 'src') . ' width="1920" height="800" title="No video playback capabilities" alt="'.esc_html__('video thumb','ratio').'" />';
	}
	$video_output .='</object>';
	$video_output .='</video>';
	$video_output .='</div>';
