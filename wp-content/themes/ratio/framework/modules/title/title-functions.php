...
if(!function_exists('ratio_edge_custom_breadcrumbs')) {
    ...
    function ratio_edge_custom_breadcrumbs() {
        ...
        $before = '<span typeof="v:Breadcrumb" class="edgtf-current" '.ratio_edge_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" style="display:none;"><a '.ratio_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a '.ratio_edge_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';

        } elseif(is_home()) {
            $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" style="display:none;">'.$before.$home.$after.'</div></div>';
        }

        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" style="display:none;"><a '.ratio_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="edgtf-breadcrumbs"><div class="edgtf-breadcrumbs-inner" style="display:none;"><span typeof="v:Breadcrumb"><a property="v:title" rel="v:url" '.ratio_edge_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></span>' . $delimiter;
            ...
            if ( is_category() || ratio_edge_is_product_category()) {
                ...
                $output .= $before .'<a property="v:title" rel="v:url" href="#content">'. single_cat_title('', false) .'</a>'. $after;
                ...
                if ( get_post_type() != 'post' ) {
                    ...
                    if ($showCurrent == 1) $output .= $before .'<a property="v:title" rel="v:url" href="#content">'. get_the_title() .'</a>'. $after;
                } else {
                ...
                $output .= '<span typeof="v:Breadcrumb"><a property="v:title" rel="v:url" '.ratio_edge_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a></span>';
                if ($showCurrent == 1) $output .= $delimiter . $before .'<span typeof="v:Breadcrumb"><a property="v:title" rel="v:url" href="#content">'. get_the_title() .'</a></span>'. $after;
                ...
                while ($parent_id) {
                    ...
                    $breadcrumbs[] = '<span typeof="v:Breadcrumb"><a property="v:title" rel="v:url" '.ratio_edge_get_inline_style($bread_style).' href="' . get_permalink($page->ID) . '">' . get_the_title($page->ID) . '</a></span>';
                ...
                if ($showCurrent == 1) $output .= $delimiter . $before .'<a property="v:title" rel="v:url" href="#content">'. get_the_title() .'</a>'. $after;
