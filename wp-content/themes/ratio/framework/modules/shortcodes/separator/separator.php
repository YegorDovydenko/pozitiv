<?php
namespace RatioEdge\Modules\Shortcodes\Separator;
...
class Separator implements ShortcodeInterface{
	...
	public function vcMap() {
		...
		vc_map(
			array(
				'name' => esc_html__('Edge Separator', 'ratio'),
				'base' => $this->base,
				'category' => 'by EDGE',
	...
	public function render($atts, $content = null) {
		...
		$html = ratio_edge_get_shortcode_module_template_part('templates/separator-template', 'separator', '', $params);
	...
	private function getSeparatorStyle($params) {
		...
		if ($params['width'] !== '') {
			if(ratio_edge_string_ends_with($params['width'], '%') || ratio_edge_string_ends_with($params['width'], 'px')) {
		...
		if ($params['top_margin'] !== '') {
			if(ratio_edge_string_ends_with($params['top_margin'], '%') || ratio_edge_string_ends_with($params['top_margin'], 'px')) {
		...
		if ($params['bottom_margin'] !== '') {
			if(ratio_edge_string_ends_with($params['bottom_margin'], '%') || ratio_edge_string_ends_with($params['bottom_margin'], 'px')) {
