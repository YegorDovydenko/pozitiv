<?php
namespace RatioEdge\Modules\Shortcodes\Button;
 ...
 * @package RatioEdge\Modules\Shortcodes\Button
 */
class Button implements ShortcodeInterface {
    ...
    public function vcMap() {
        vc_map(array(
            'name'                      => esc_html__('Edge Button', 'ratio'),
            'base'                      => $this->base,
            'category'                  => 'by EDGE',
            ...
            'params'                    => array_merge(
                ...
                ratio_edge_icon_collections()->getVCParamsArray(array(), '', true),
                array(
                    array(
						'type'        => 'colorpicker',
						'heading'     => 'Color',
                    ...
                    array(
                        ...
                        'admin_label' => true,
                        'dependency'  => array('element' => 'type', 'value' => array('solid','gradient')),
                        'group'       => 'Design Options'
                    ),
					array(
						'type'        => 'colorpicker',
						'heading'     => 'Background Second(Gradient) Color',
						'param_name'  => 'background_second_color',
						'admin_label' => true,
                    ...
                    array(
                        ...
                        'value'       => array_flip(ratio_edge_get_font_weight_array(true)),
                        'admin_label' => true,
                        'group'       => 'Design Options'
                    ),
                    array(
                        ...
                        'heading'     => 'Margin',
                        'param_name'  => 'margin',
                        'description' => esc_html__('Insert margin in format: 0px 0px 1px 0px', 'ratio'),
                        'admin_label' => true,
                        'group'       => 'Design Options'
                    )
                )
    ...
    public function render($atts, $content = null) {
        ...
        $params['button_background_styles'] = $this->getButtonBackgroundStyles($params);
        $params['button_background_hover_styles'] = $this->getButtonBackgroundHoverStyles($params);
        $params['button_data']         = $this->getButtonDataAttr($params);

		$icon_attributes['class'] = 'edgtf-btn-icon-holder';
		$icon_params_array['icon_attributes'] = $icon_attributes;
		$icon_params_array['icon_attributes']['style'] = $this->generateIconStyles($params);
		$params['icon_params_array'] = $icon_params_array;

        return ratio_edge_get_shortcode_module_template_part('templates/'.$params['html_type'], 'button', $params['hover_animation'], $params);
    }
    ...
    private function getButtonStyles($params) {
        ...
        if(!empty($params['font_size'])) {
            $styles[] = 'font-size: '.ratio_edge_filter_px($params['font_size']).'px';
        }
    ...
    /**
     * Returns array of button background styles
     *
     * @param $params
     *
     * @return array
     */
    private function getButtonBackgroundStyles($params) {
        $styles = array();

        if(!empty($params['background_color']) && $params['type'] !== 'outline') {
            $styles[] = 'background-color: '.$params['background_color'];

			if(!empty($params['background_second_color']) && $params['type'] == 'gradient') {
				$styles[] = ratio_edge_inline_background_gradient($params['background_color'], $params['background_second_color']);
			}
        }
     ...
     * @return array
     */
    private function getButtonBackgroundHoverStyles($params) {
        $styles = array();
        ...
        if(!empty($params['hover_border_color'])) {
            $styles[] = 'border: 1px solid '.$params['hover_border_color'];
        }
    ...
    private function getButtonClasses($params) {
        ...
        if(!empty($params['hover_background_color']) || !empty($params['hover_border_color']) || ($params['type'] !== 'transparent')) {
            $buttonClasses[] = 'edgtf-has-hover';
        }
