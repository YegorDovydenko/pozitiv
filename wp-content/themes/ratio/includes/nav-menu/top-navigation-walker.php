 ...
 */
if (!class_exists('RatioEdgeTopNavigationWalker')) {
	class RatioEdgeTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				...
				if (ratio_edge_options()->getOptionValue('enable_wide_menu_background') == "yes"){
					$classes[] = 'edgtf-wide-background';
				}
			}
			...
			if($item->hide == ""){
				...
				if($args->has_children && $depth == 1) {
					$item_output .= '<i class="edgtf-menu-arrow fa fa-angle-right"></i>';
				}
				...
				$item_output .= '</a>'; //close span.edgtf-item-outer
			}
